...
class RelationalDataLoader:
    ...
    def main(self):
        ...
        Providers.AuditColumnsNames.update_audit_column_prefix(
            self.args.audit_column_prefix
        )

        self.args.func()

    def execute_process_command(self):
        source_db = self.data_source_factory.create_source(
            self.args.source_connection_string
        )
        ...
        data_load_manager = DataLoadManager(
            self.args.configuration_folder, source_db, destination_db, repository
        )
        total_rows_processed = data_load_manager.start_imports(
            self.args.force_full_refresh_models
        )
    ...
    def execute_audit_command(self):
        ...
        results = _AUDIT_FUNCTION_OPTIONS[self.args.model_type](
            data_load_tracker_repository, last_successful_timestamp
        )
    ...
    def raw_connection_string_to_valid_source_connection_string(
    ...
    def get_arguments(self):
        ...
        process_command_parser = subparsers.add_parser(
            "process", help="processes load models"
        )
        ...
        audit_command_parser = subparsers.add_parser(
            "audit", help="provides list of processed models since a given timestamp"
        )
