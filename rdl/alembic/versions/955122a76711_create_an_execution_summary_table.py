...
def upgrade():
    ...
    op.execute(
        "ALTER TABLE rdl.data_load_execution RENAME COLUMN correlation_id TO execution_id"
    )
    op.execute(
        """
    ...
    op.execute(
        "UPDATE rdl.data_load_execution SET status = 'Successful' WHERE status = 'Completed Successfully'"
    )
...
def downgrade():
    ...
    op.execute(
        "ALTER TABLE rdl.data_load_execution DROP CONSTRAINT pk_data_load_execution"
    )
    ...
    op.add_column(
        ...
        sa.Column(
            ...
            server_default=sa.text(
                "nextval('rdl.data_load_execution_id_seq'::regclass)"
            ),
            autoincrement=True,
            nullable=False,
        ),
    ...
    op.create_primary_key(
        "pk_data_load_execution", "data_load_execution", ["id"], schema="rdl"
    )

    op.execute(
        "ALTER SEQUENCE rdl.data_load_execution_id_seq OWNED BY rdl.data_load_execution.id"
    )
    ...
    op.execute(
        "UPDATE rdl.data_load_execution SET status = 'Completed Successfully' WHERE status = 'Successful'"
    )
    op.execute(
        "ALTER TABLE rdl.data_load_execution RENAME COLUMN execution_id TO correlation_id"
    )
